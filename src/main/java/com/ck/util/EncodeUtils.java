...
public class EncodeUtils {
    ...
    public static String BCD2ASC(byte[] bytes) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            ...
            int l = (bytes[i] & 0x0f);
            temp.append(ascii[h]).append(ascii[l]);
        }
    ...
    public static byte[] hexStringToByteForC(String hex) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            // 1]));
            if ((achar[pos] <= '9') && (achar[pos] >= '0')) {
                hightByte = (byte) (achar[pos] & 0x0f);

            } else {
                hightByte = (byte) ((achar[pos] & 0x0f) + 9);
            }

            if ((achar[pos + 1] <= '9') && (achar[pos + 1] >= '0')) {
                lowByte = (byte) (achar[pos + 1] & 0x0f);
            } else {
                lowByte = (byte) ((achar[pos + 1] & 0x0f) + 9);
            }
            result[i] = (byte) ((hightByte << 4) & 0xf0 | lowByte);
        }
        return result;
    }
